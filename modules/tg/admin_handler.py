...
async def admin_start_handler(message: Message) -> None:
    ...
    logging.info(f"Admin {message.from_user.id} used the ADMIN comand /start")

@admin_router.message(IsAdmin(), Command('admin'))
async def admin_handler(message: Message) -> None:
    await message.answer("Админ панель:\n\n/logs - отправить лог-файл")
    logging.info(f"Admin {message.from_user.id} used the ADMIN comand /admin")
...
async def log_sender(message: Message) -> None:
    ...
    logging.info(f"Admin {message.from_user.id} used the ADMIN comand /logs")